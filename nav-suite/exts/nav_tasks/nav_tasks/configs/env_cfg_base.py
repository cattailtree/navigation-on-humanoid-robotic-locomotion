...
# Scene definition
##
def nav_or_generated_commands(env: ManagerBasedRLEnv, command_name: str):
    """优先使用 NavigationSE2Action 写进 env 的 navigation_commands；
    否则退回原来的 mdp.generated_commands（也就是 MixedCommand 那套）。
    """
    # 如果 NavigationSE2Action 已经把 SE2 命令挂到 env.navigation_commands
    if hasattr(env, "navigation_commands") and command_name == "base_velocity":
        return env.navigation_commands

    # 否则用原来的命令生成系统
    return mdp.generated_commands(env, command_name=command_name)
...
class ObservationsCfg:
    ...
    class LocomotionPolicyCfg(ObsGroup):
        """低层 G1 行走 policy 用的 obs（结构和训练时一致）"""
        ...
        # 这里用 nav_or_generated_commands，把 NavigationSE2Action 的 SE2 命令塞进来
        velocity_commands = ObsTerm(
            func=nav_or_generated_commands,
    ...
    # 这个名字就是 obs group 的名字，后面要在 NavigationSE2ActionCfg.low_level_obs_group 里用到
