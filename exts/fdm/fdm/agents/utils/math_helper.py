...
class Normalizer:
    ...
    def load(self, results_dir: str | pathlib.Path):
    ...
    def save(self, save_dir: str | pathlib.Path):
        """Saves stored statistics to the given path."""
        save_dir = pathlib.Path(save_dir)
        with open(save_dir / self._STATS_FNAME, "wb") as f:
            pickle.dump({"mean": self.mean.cpu().numpy(), "std": self.std.cpu().numpy()}, f)


# ------------------------------------------------------------------------ #
# Uncertainty propagation functions
# ------------------------------------------------------------------------ #
def propagate_from_indices(predicted_tensor: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
...
def propagate_random_model(predictions: tuple[torch.Tensor, ...]) -> tuple[torch.Tensor, ...]:
    ...
    for i, predicted_tensor in enumerate(predictions):
        ...
        model_indices = torch.randint(num_models, size=(batch_size,), device=predicted_tensor.device)
        output.append(propagate_from_indices(predicted_tensor, model_indices))
    return tuple(output)


def propagate_expectation(predictions: tuple[torch.Tensor, ...]) -> tuple[torch.Tensor, ...]:
...
def propagate(
    ...
    propagation_indices: torch.Tensor | None = None,
) -> tuple[torch.Tensor, ...]:
...
def quantize_obs(
    ...
    if add_noise:
        quantized_obs = quantized_obs.astype(np.double) + ratio * np.random.rand(*obs.shape)
