...
# https://github.com/facebookresearch/mbrl-lib/blob/main/mbrl/util/math.py
# https://github.com/facebookresearch/mbrl-lib/blob/main/mbrl/planning/trajectory_opt.py
...
class TrajectoryOptimizer:
    ...
    def __init__(
        ...
        self.initial_solution = self.initial_solution.repeat((planning_horizon, 1)) * 0.0
...
class BatchedICEMOptimizer(Optimizer):
    ...
    def optimize(
        ...
        obj_fun: Callable[[torch.Tensor], torch.Tensor],
        x0: Optional[torch.Tensor] = None,
        var0: Optional[torch.Tensor] = None,
        callback: Optional[Callable[[torch.Tensor, torch.Tensor, int], None]] = None,
        **kwargs,
    ) -> torch.Tensor:
        """Runs the optimization using iCEM.
        ...
        """
        if x0 is None:
            x0 = torch.zeros((1, self.planning_horizon, self.lower_bound.shape[1]), device=self.device)
            # (
            #     powerlaw_psd_gaussian(
            #         1.0,
            #         size=(BS, self.upper_bound.shape[0], self.planning_horizon),
            #         device=self.device,
            #     )
            #     .permute(0, 2, 1)
            #     .contiguous()
        ...
        # store final cost for high risk filtering
        self.selected_value = best_value
    ...
    def optimize(
        ...
        **kwargs,
        ...
        for k in range(self.refinements):
            ...
            values = obj_fun(population[:, None, :, :])
            values = values[:, 0]
            ...
            weights = torch.reshape(
                torch.exp(self.gamma * (values - values.max())),
                (self.population_size + int(self.provide_zero_action), 1, 1),
            )
            norm = torch.sum(weights) + 1e-10
            weighted_actions = population * weights
            self.mean = torch.sum(weighted_actions, dim=0) / norm

            self.selected_value = (values * weights.squeeze()).sum() / norm

        return self.mean.clone()[None], None
