...
from fdm_navigation.cfg import RobotCfg
...
def get_robot_shape(cfg: RobotCfg, device):
    """
    Returns the indices of the robot shape in the grid map

    fatal_xy (torch.Size([Nf, 2]), torch.float32): Indices of the fatal area
    risky_xy (torch.Size([Nr, 2]), torch.float32): Indices of the risky area
    cautious_xy (torch.Size([Nc, 2]), torch.float32): Indices of the cautious area
    """
